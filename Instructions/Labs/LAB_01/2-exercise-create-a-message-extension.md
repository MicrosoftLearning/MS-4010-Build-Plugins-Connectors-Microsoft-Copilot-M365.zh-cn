...
![Microsoft Teams 中基于搜索的消息扩展返回的搜索结果屏幕截图。](../media/2-search-results-nuget.png)
...
1. 在文件中，找到使用 botAadApp/create**** 操作（第 17-26 行）的步骤，并将其替换为：
...
1. 在文件中，删除使用 botFramework/create**** 操作（第 53-62 行）的步骤。
1. 保存所做更改。
...
![Visual Studio 中展开的“开发隧道”菜单的屏幕截图。](../media/18-select-dev-tunnel.png)

在 Visual Studio 中继续操作：

1. 在工具栏上，确保将 MsgExtProductSupport**** 选为启动项目，并通过选择 **Microsoft Teams（浏览器）按钮**或**启动项目**旁边的下拉列表来展开调试配置文件菜单。
1. 展开 **“开发隧道”（无活动隧道）** 菜单，并选择 **“创建隧道...”**
1. 在对话框中，指定以下值：
    1. **帐户**：使用 Microsoft 365 用户帐户登录。
...
![用于本地开发的颜色图标。](../media/app/color-local.png)

![用于远程开发的颜色图标。](../media/app/color-dev.png)
...
1. 在文件中，将**图标**、**名称**和**说明**对象替换为以下内容（第 13-24 行）：
...
1. 当系统要求信任不同的 SSL 证书时，请选择“是”****，然后再次选择“是”**** 以显示任何安全警告。 请注意，在接受证书后，可能需要重启调试器。
